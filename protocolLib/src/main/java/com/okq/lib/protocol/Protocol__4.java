 ...
 */
public class Protocol__4 extends Protocol {
    ...
    public String createModifyAddressBytes(String oldAddress, String newAddress) throws ProtocolException {
        ...
        String data = oldAddress + "060B0000" + newAddress;
        data += crc16(data);
        return data;
    }
    ...
    public String decodeReadAddressResult(String data) throws ProtocolException {
        throw new ProtocolException("此协议暂不支持查询地址");
    }

    @Override
    public String decodeModifyAddressResult(String data) throws ProtocolException {
        if (!data.matches("^([A-F0_9]{2})25[A-F0_9]{2}[A-F0_9]+[A-F0_9]{4}"))
            throw new ProtocolException("数据格式不对");
        String length = data.substring(4, 6);
        String remaining = data.substring(6);
        if (remaining.length() / 2 != Integer.parseInt(length, 16) + 2)
            throw new ProtocolException("数据长度校验失败");
        String crc = crc16(data.substring(0, data.length() - 4));
        if (!crc.equals(data.substring(data.length() - 4)))
